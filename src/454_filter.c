...
int main (int argc, const char * argv[]) 
{
	...
	if (argc>=7)
	{
		aux1 = sscanf (argv[6],"%255s",tag);
		if (aux1 == EOF)
		{
			fprintf (stderr, "Failed to proces the tag argument %s\n", argv[6]);
			return 1;
		}
			...
			else
			{
				if (currentChar == '>' || isFEOF)
				{
					...
					else
					{
						...
						{
							aux3 = scores->vLength + 1;
							if (seqData->sLength > tagLength + min_length)
							{
								for (aux3 = 0; aux3 < tagLength; aux3++)
									if (seqData->sData[aux3] != tag[aux3] )
									{
										if (aux4++ >= tagMismatch)
											break;
									}
